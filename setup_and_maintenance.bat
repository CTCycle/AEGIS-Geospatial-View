...
set "init_db_module=scripts.initialize_database"
set "gibs_module=scripts.update_gibs_layers"
...
if exist "%uv_lock%" (
  ...
  echo [INFO] No runtime uv.lock file found to remove.
)
if exist "%uv_dir%" (
  rd /s /q "%uv_dir%"
...
if exist "%venv_dir%" (
  ...
  echo [INFO] No runtime .venv directory found to remove.
)
if exist "%client_dir%\node_modules" (
  rd /s /q "%client_dir%\node_modules"
