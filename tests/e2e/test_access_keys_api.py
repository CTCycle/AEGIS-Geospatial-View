...
def test_access_keys_crud_and_prefix_parity(api_context: APIRequestContext) -> None:
    ...
    activate_base = api_context.put(f"/api/access-keys/{key_id}/activate{_provider_query(provider)}")
    ...
    delete_base = api_context.delete(f"/api/access-keys/{key_id}{_provider_query(provider)}")
...
def test_access_keys_invalid_provider_and_unknown_id(api_context: APIRequestContext) -> None:
    ...
    missing_activate = api_context.put("/api/access-keys/999999/activate?provider=openai")
