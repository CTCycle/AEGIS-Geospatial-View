...
def test_access_keys_crud_and_prefix_parity(api_context: APIRequestContext) -> None:
    ...
    create_base = api_context.post("/access-keys", data=create_payload)
    ...
    list_base = api_context.get(f"/access-keys{_provider_query(provider)}")
    ...
    activate_base = api_context.put(f"/access-keys/{key_id}/activate{_provider_query(provider)}")
    activate_prefixed = api_context.put(f"/api/access-keys/{key_id}/activate{_provider_query(provider)}")
    ...
    delete_base = api_context.delete(f"/access-keys/{key_id}{_provider_query(provider)}")
    delete_prefixed = api_context.delete(f"/api/access-keys/{key_id}{_provider_query(provider)}")
    assert delete_base.ok
    assert delete_prefixed.status in {200, 404}


def test_access_keys_invalid_provider_and_unknown_id(api_context: APIRequestContext) -> None:
    invalid = api_context.get("/access-keys?provider=unknown_provider")
    assert invalid.status == 422

    missing_activate = api_context.put("/access-keys/999999/activate?provider=openai")
    missing_delete = api_context.delete("/access-keys/999999?provider=openai")
