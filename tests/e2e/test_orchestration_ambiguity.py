...
def test_ambiguous_temporal_request_produces_clarification(api_context: APIRequestContext) -> None:
    body = _turn(api_context, "Show me weather")
    assert body.get("follow_up_required") is True
    assert body.get("map_session") is None


def test_missing_location_produces_clarification_or_validation(api_context: APIRequestContext) -> None:
...
def test_direct_coordinates_request_does_not_create_map_session(api_context: APIRequestContext) -> None:
...
def test_runtime_overlay_request_reply_does_not_leak_internal_tool_ids(api_context: APIRequestContext) -> None:
...
def test_missing_key_request_clarifies_or_falls_back_consistently(api_context: APIRequestContext) -> None:
    body = _turn(api_context, "Use TomTom traffic in Rome")
    assistant = str(body.get("assistant_message") or "").lower()
    if body.get("follow_up_required") is True:
        assert "key" in assistant or "configure" in assistant or "alternative" in assistant
        return
