...
def test_access_keys_router_status_codes_and_domain_exception_mappings(monkeypatch) -> None:
    ...
    client = _app()
    assert client.post("/access-keys", json={"provider": "openai", "access_key": "sk-value"}).status_code == 201
...
def test_access_keys_router_422_and_404_translation_paths(monkeypatch) -> None:
    monkeypatch.setattr(serializer, "create_key", lambda provider, access_key: (_ for _ in ()).throw(ValueError("bad provider")))
    monkeypatch.setattr(serializer, "activate_key", lambda key_id, provider: (_ for _ in ()).throw(KeyError("missing")))
    monkeypatch.setattr(serializer, "delete_key", lambda key_id, provider: (_ for _ in ()).throw(KeyError("missing")))
    monkeypatch.setattr(serializer, "list_keys", lambda provider: (_ for _ in ()).throw(ValueError("bad provider")))
