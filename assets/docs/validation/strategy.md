...
The latest per-slice roll-up remains `PARTIAL`: 13 slices are `PASS`, 4 are
...
and a focused transcript-spacing repair. Tier 1 is `PARTIAL` at 8 `PASS` and
4 `PARTIAL`; downstream live/browser/provider/hosted-CI boundaries remain
open. Continue at `T1-06`.
