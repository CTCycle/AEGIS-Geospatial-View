...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `runtime.startup.windows-local` | `PARTIAL` | Windows launcher port-consent guard, warm dependency/build reuse, concurrent backend/frontend startup, and isolated no-op database startup. | The official launcher smoke at `develop@66008da` reached backend and frontend health with the isolated runtime; see the [exact-head recheck report](../QA/tier2-validation-develop-20260924-head66008da/report.md). Prior checks include 401 backend, 248 Angular, and 19 frontend-state tests; see the [startup optimization report](../QA/startup-optimization-20260922/report.md). | No paired same-machine pre-change timing; dynamic-provider outage, protected-process/ownership-change races, and injected service-failure cleanup remain unverified. Angular `ng serve` still compiles at launch. | — | 2026-09-24 | E2E + automated | [startup](runtime/startup.md), [runtime modes](runtime/modes.md), [optimization report](../QA/startup-optimization-20260922/report.md), [exact-head recheck report](../QA/tier2-validation-develop-20260924-head66008da/report.md) | Capture paired before/after warm timings and exercise provider-outage and failed-start cleanup paths. |
...
| `agent.capability-routing.live-language` | `PARTIAL` | Natural-language routing and capability discovery for keyless geospatial requests. | The 2026-09-23 exact-lane T2 retry fixed POI map routes missing `DATA_RETRIEVAL`; the 2026-09-24 exact-head focused suite adds 103 passing routing, location-resolution, and agent-loop tests on `66008da`. The related exact-lane browser recheck was blocked before request execution because the isolated runtime had no selected model or OpenCode Go key. | `ISSUE-006` and remaining T2 gaps: Acropolis semantics, imagery extent, weather budget, USGS/land-cover aliases, generic-infrastructure clarification, and Milan candidate quality remain open. Overpass itself reports partial reliability/limited extent. | — | 2026-09-24 | E2E + automated | [exact-head recheck report](../QA/tier2-validation-develop-20260924-head66008da/report.md), [focused backend log](../QA/tier2-validation-develop-20260924-head66008da/focused-backend.log), [2026-09-23 T2 execution](../QA/tier2-validation-develop-20260923-retry/report.md) | Re-run the exact-lane location and route flows when the isolated runtime has the configured provider; complete the remaining route matrix and preserve source, route, coverage, and render evidence. |
| `agent.location-resolution.core` | `VALIDATED` | Direct locations, clarification, invalid-coordinate bounds, safe recovery, and repeated location changes. | Current T2-01 browser evidence includes Zurich with bounds rejection retaining the map, Springfield clarification, and Milan rendered after explicit selection. The explicit-country resolver regression rejects Tehran/Iran for Rome, Italy. | `Mostrami Milano` still offers an incongruous Texas alternative and the selected Italy result carries a `Rodano` geocoder label; see `T2-01 PARTIAL`. | — | 2026-09-23 | E2E | [core report](../QA/core-behavior-e2e-20260919/final-report.md), [agentic search](geospatial/agentic_search.md), [T2 retry report](../QA/tier2-validation-develop-20260923-retry/report.md) | Improve/revalidate candidate quality while preserving the explicit-country and invalid-bounds safeguards. |
...
| Component | Status | Scope | Evidence | Known Issues | Blocker | Last Validated | Validation Level | Related Docs | Next Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ui.chat-map-workspace` | `VALIDATED` | Startup-to-chat flow, clarification, map controls, transcript hygiene, recovery, and repeated core location use. | Current T2 browser evidence shows Rome transcript/map hydration after reload, explicit Florence render acknowledgement, and a visible, attributed POI layer. | `Show me Florence on the map.` still replaces Rome without clarification (`T2-02 PARTIAL`); `Mostrami Milano` has poor candidate labels (`T2-01 PARTIAL`). DSML protocol-looking finalizer content is suppressed. | — | 2026-09-23 | E2E | [frontend architecture](architecture/frontend_architecture.md), [core report](../QA/core-behavior-e2e-20260919/final-report.md), [T2 retry report](../QA/tier2-validation-develop-20260923-retry/report.md) | Close the location-confirmation and Milan candidate-quality gaps, then rerun the broader browser smoke. |
...
| `validation.application-foundations-tier1` | `VALIDATED` | Tier 1 route, state, conversation, realtime, run, API, jobs, Settings, credential, model, and context foundations. | T1-10 passed on `develop@8e32f82e3f094e5fb17c3978fad69b9f80b7af0b`; Tier 1 remains 12 `PASS`, and current campaign roll-up is 18 `PASS` / 2 `PARTIAL` / 0 `BLOCKED` / 48 `UNRUN`. The [2026-09-24 exact-head T2 recheck](../QA/tier2-validation-develop-20260924-head66008da/report.md) records current downstream blockers and 103 passing focused routing tests. Hosted CI run [36003422893](https://github.com/CTCycle/AEGIS-Geospatial-View/actions/runs/36003422893) passed all four jobs on `develop@0113812c7e4e1945c9a2846bec1942c6889a7264`. | Milan candidate quality and ambiguous Florence confirmation remain `PARTIAL`; broader downstream live/browser/provider coverage remains open. | — | 2026-09-24 | integration/E2E | [validation strategy](validation/strategy.md), [exact-head T2 recheck](../QA/tier2-validation-develop-20260924-head66008da/report.md), [Tier 1 checklist](validation/tier1_application_foundations.md), [T1-10 report](../QA/tier1-validation-develop-20260923/T1-10/report.md), [T1-09 report](../QA/tier1-validation-develop-20260923/T1-09/report.md), [2026-09-23 T1-07 report](../QA/tier1-validation-develop-20260923/T1-07/report.md), [T1-06 report](../QA/tier1-validation-develop-20260923/T1-06/report.md), [T1-03 report](../QA/tier1-validation-develop-20260923/T1-03/report.md), [2026-09-22 T1-02 report](../QA/tier1-validation-develop-20260922/T1-02/report.md), [historical Tier 1 baseline](../QA/tier1-application-foundations-20260921/ledger.md) | Complete the T2 partials and unrun slices while keeping browser, provider, and exact-head CI boundaries distinct. |
...
| `validation.environment-startup-tier0` | `VALIDATED` | Tier 0 environment, current SQLite/Alembic schema, legacy Settings migration, Windows startup, and API composition slices. | All five Tier 0 slices remain passing; the campaign is now 18 `PASS`, 2 `PARTIAL`, 0 `BLOCKED`, and 48 `UNRUN`. The official launcher reached backend/frontend health with the isolated test root at `develop@66008da`; this supplements the startup smoke evidence and does not claim a new comprehensive T0 pass. | T2-01/02 are `PARTIAL`, T2-03 is `PASS`, and other downstream live/browser/provider validation remains open. | — | 2026-09-24 | integration | [Tier 0 final report](../QA/tier0-validation-develop-20260922/final/report.md), [T1-10 report](../QA/tier1-validation-develop-20260923/T1-10/report.md), [validation strategy](validation/strategy.md), [gate ledger](validation/gate_status.md), [exact-head T2 report](../QA/tier2-validation-develop-20260924-head66008da/report.md) | Complete the location partials and unrun T2 slices; retain all other partial, blocked, and unrun boundaries. |
| `testing.complete-live-matrix` | `PARTIAL` | Complete live provider, browser, raster, composition, and coverage scenario matrix. | At `develop@66008da`, the controlled acknowledgement module passes 10/10 and the focused routing suite passes 103/103, while the exact-lane T2 recheck was blocked before any provider request. The 2026-09-23 T2 execution and API conflict evidence remain the latest live workflow evidence. | Explicit gaps remain for raster loading, route aliases and semantics, provider availability, coverage reruns, and other catalog families. | External providers and configured sources vary by environment; the isolated exact lane was not configured for this recheck. | 2026-09-24 | E2E | [exact-head recheck report](../QA/tier2-validation-develop-20260924-head66008da/report.md), [controlled module](../QA/tier2-validation-develop-20260924-head66008da/controlled-fault/full-module.log), [2026-09-23 T2 retry](../QA/tier2-validation-develop-20260923-retry/report.md), [focused report](../QA/aegis-geospatial-e2e-validation-20260920/report.md), [hazard rerun](../QA/aegis-hazard-hydrology-e2e-20260920/report.md) | Execute the remaining rows with the exact provider/model and browser-authoritative evidence. |
...
| `maps.fema-coverage-guardrail` | `UNVALIDATED` | The earlier Zurich result remains inconclusive. The 2026-09-24 exact-lane follow-up was blocked before request execution because the isolated runtime had no selected model or OpenCode Go key; the guardrail was not run. | `HIGH` | Re-run the guardrail when the exact lane is configured and prove one clarification/rejection without provider or map execution. |
