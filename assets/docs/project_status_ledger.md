...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `runtime.startup.windows-local` | `VALIDATED` | Official Windows launcher starts the backend and frontend, waits for health/UI readiness, and supports the tested local workflow. | Core browser run completed startup and verified ports 7059 and 4512. | No Linux or macOS launcher; no standalone production distribution. | — | 2026-09-19 | E2E | [startup](runtime/startup.md), [runtime modes](runtime/modes.md), [core report](../QA/core-behavior-e2e-20260919/final-report.md) | Re-run launcher health and cleanup after launcher or configuration changes. |
...
| `ui.settings-provider-access` | `UNVALIDATED` | Settings tabs for model selection, model-provider credentials, local runtime, and geospatial access. | Architecture and access contracts exist, but the current QA reports do not contain a recent complete browser pass for this area. | No current functional defect is asserted; confidence is insufficient. | — | — | None | [frontend architecture](architecture/frontend_architecture.md), [access overview](geospatial/providers/access_overview.md), [user settings](user/settings_and_access.md) | Perform a manual browser pass for each settings tab, masked credential update, readiness state, and failure state. |
...
| `ui.settings-provider-access` | `UNVALIDATED` | Complete browser coverage of model, provider, geospatial-access, masked-secret, and unavailable states. | `MEDIUM` | Run the settings workflow manually with safe fixture credentials or explicit missing-access states. |
