...
| `validation.application-foundations-tier1` | `PARTIAL` | Tier 1 route, state, conversation, realtime, run, API, jobs, Settings, credential, model, and context foundations. | T1-06 passed the exact-lane live response matrix and corrected backend CI selection. The roll-up is 9 `PASS` / 3 `PARTIAL` within Tier 1 and 14 `PASS` / 3 `PARTIAL` / 51 `UNRUN` across the campaign; evidence boundaries remain slice-specific. | T1-07 mounted jobs/shutdown; T1-09 test-contract drift; T1-10 credential failure fixtures. | — | 2026-09-23 | integration/E2E | [validation strategy](validation/strategy.md), [Tier 1 checklist](validation/tier1_application_foundations.md), [2026-09-23 T1-06 report](../QA/tier1-validation-develop-20260923/T1-06/report.md), [T1-03 report](../QA/tier1-validation-develop-20260923/T1-03/report.md), [2026-09-22 T1-02 report](../QA/tier1-validation-develop-20260922/T1-02/report.md), [historical Tier 1 baseline](../QA/tier1-application-foundations-20260921/ledger.md) | Continue with T1-07; keep downstream and hosted-CI claims separate. |
...
| `validation.environment-startup-tier0` | `VALIDATED` | Tier 0 environment, current SQLite/Alembic schema, legacy Settings migration, Windows startup, and API composition slices. | All five Tier 0 slices remain passing; the latest campaign reconciliation records 14 `PASS`, 3 `PARTIAL`, and 51 `UNRUN` after T1-06. | Downstream live/browser/provider/hosted-CI validation remains separate and incomplete. | — | 2026-09-23 | integration | [Tier 0 final report](../QA/tier0-validation-develop-20260922/final/report.md), [T1-06 report](../QA/tier1-validation-develop-20260923/T1-06/report.md), [validation strategy](validation/strategy.md), [gate ledger](validation/gate_status.md) | Continue at T1-07; retain downstream partial and unrun boundaries. |
...
| `ci.hosted-exact-head` | `PARTIAL` | Hosted CI result for an exact pushed branch head. | The prior push run for `fccafa1f48be71a8f68116b83783b29369e9e419` failed because its backend pytest command referenced missing path `app/tests/unit/services/search`. This change removes that target, and the corrected backend selection passes locally (397 tests); hosted CI for the exact implementation commit is pending. | Hosted backend coverage for the corrected pushed head is not established until its Actions run passes. | — | 2026-09-23 | hosted CI | [gate status](validation/gate_status.md), [T1-06 report](../QA/tier1-validation-develop-20260923/T1-06/report.md), [previous failure record](../QA/tier1-validation-develop-20260923/T1-03/hosted-ci.md), [testing and quality](coding/testing_and_quality.md) | Inspect the exact pushed-head CI run; keep this row partial until all required jobs pass. |
