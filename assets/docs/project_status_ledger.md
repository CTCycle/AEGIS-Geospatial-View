...
| `backend.api.sync-chat-turn` | `PARTIAL` | Synchronous chat-turn response boundary and orchestration smoke behavior. | Current isolated composition tests prove terminal `200 ChatTurnResponse`, accepted non-terminal `202 AgentRunAcceptedResponse`, and `409` only for a genuine active-run/idempotency/ownership conflict. | The broader live-provider timing boundary remains outside this local composition proof; retain `ISSUE-003` until the live contract is rerun. | — | 2026-09-22 | integration | [T0-05 report](../QA/tier0-validation-develop-20260922/T0-05/report.md), [backend API](architecture/backend_api.md), [gate status](validation/gate_status.md) | Revalidate the current 200/202/409 contract on the live provider lane; do not restore the historical generic-409 expectation. |
...
| `validation.application-foundations-tier1` | `PARTIAL` | Tier 1 route, state, conversation, realtime, run, API, jobs, Settings, credential, model, and context foundations. | T1-03 passed focused API/repository tests, the geospatial-page Karma suite, and the in-app Browser search, pagination, hydration, isolation, and stale-ID flow. The roll-up is 8 `PASS` / 4 `PARTIAL` within Tier 1 and 13 `PASS` / 4 `PARTIAL` / 51 `UNRUN` across the campaign; evidence boundaries remain slice-specific. | T1-06 live HTTP timing; T1-07 mounted jobs/shutdown; T1-09 test-contract drift; T1-10 credential failure fixtures. | — | 2026-09-23 | integration/E2E | [validation strategy](validation/strategy.md), [Tier 1 checklist](validation/tier1_application_foundations.md), [2026-09-23 T1-03 report](../QA/tier1-validation-develop-20260923/T1-03/report.md), [2026-09-22 T1-02 report](../QA/tier1-validation-develop-20260922/T1-02/report.md), [historical Tier 1 baseline](../QA/tier1-application-foundations-20260921/ledger.md) | Continue with T1-06; keep downstream and hosted-CI claims separate. |
...
| `validation.environment-startup-tier0` | `VALIDATED` | Tier 0 environment, current SQLite/Alembic schema, legacy Settings migration, Windows startup, and API composition slices. | All five Tier 0 slices remain passing; the latest campaign reconciliation records 13 `PASS`, 4 `PARTIAL`, and 51 `UNRUN` after T1-03. | Downstream live/browser/provider/hosted-CI validation remains separate and incomplete. | — | 2026-09-22 | integration | [Tier 0 final report](../QA/tier0-validation-develop-20260922/final/report.md), [T1-03 report](../QA/tier1-validation-develop-20260923/T1-03/report.md), [validation strategy](validation/strategy.md), [gate ledger](validation/gate_status.md) | Continue at T1-06; retain downstream partial and unrun boundaries. |
...
| `ci.hosted-exact-head` | `BROKEN` | Hosted CI result for an exact pushed branch head. | The push run for `fccafa1f48be71a8f68116b83783b29369e9e419` failed in `backend-unit-tests` before collection because the configured command references missing path `app/tests/unit/services/search`; the other three jobs passed. | Hosted backend unit coverage is not established until the stale workflow path is corrected and rerun. This separate CI failure does not change the locally validated T1-03 result. | — | 2026-09-23 | hosted CI | [gate status](validation/gate_status.md), [T1-03 hosted-CI record](../QA/tier1-validation-develop-20260923/T1-03/hosted-ci.md), [testing and quality](coding/testing_and_quality.md) | Correct the missing workflow test path and rerun hosted CI on the exact pushed head. |
...
- **Severity:** `MEDIUM`.
- **Description and impact:** Historical live evidence recorded a typed 409
  while `/api/chat/turn` was still running. The current documented contract
  now reserves 409 for genuine active-run, idempotency, or ownership conflicts
  and uses typed 202 acceptance for non-terminal work.
- **Evidence:** The current isolated 200/202/409 composition proof is recorded
  in the [T0-05 report](../QA/tier0-validation-develop-20260922/T0-05/report.md);
  the historical live observation remains in the [gate ledger](validation/gate_status.md).
- **Suspected cause:** The live-provider timing boundary has not yet been
  rerun under the current contract; no stronger cause is asserted.
- **Blocker:** None.
- **Remediation status:** Local contract revalidated; live-provider
  revalidation remains open.
- **Required revalidation:** Run the current 200/202/409 contract on the live
  provider lane, including terminal hydration, active-run/idempotency/ownership
  conflict, error, and cancellation behavior. Do not mechanically restore the
  historical generic-409 expectation.
