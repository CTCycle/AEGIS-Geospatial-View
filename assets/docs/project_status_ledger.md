...
The 2026-09-26 Tier 3 follow-up fixed named-landmark location-only recovery and
manual basemap warning cleanup, then passed the current Colosseum, USGS gauge,
layer-visibility, Dark, and OpenTopoMap browser slices. A current NOAA alert
request returned non-renderable evidence and stopped after repeated invalid
tool calls, so that boundary is recorded as `BLOCKED`; the remaining Tier 3
slices are still unrun. See the [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md).
...
| `agent.capability-routing.live-language` | `PARTIAL` | Natural-language routing and capability discovery for keyless geospatial requests. | The 2026-09-25 focused five-file regression passes 83 tests in this follow-up, while the 2026-09-26 route regression adds the cleared-secondary-domain location-only normalization. The exact lane now passes current Colosseum recovery and USGS gauge routing; the bounded 50-candidate discovery inventory, direct tools, saved-history hydration, and saved-evidence inspection remain separately evidenced. | `ISSUE-006` and remaining gaps: Acropolis semantics, imagery extent, weather budget, USGS/land-cover aliases, generic-infrastructure clarification, broader route-family coverage, and the current NOAA non-renderable/model-call boundary. Overpass reports partial reliability and bounded coverage. | — | 2026-09-26 | E2E + automated | [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md), [browser evidence](../QA/tier3-validation-develop-20260926-map-rendering/browser-evidence.md), [follow-up report](../QA/tier0-tier2-validation-develop-20260925-t0-t2-followup/report.md), [focused suite](../QA/tier0-tier2-validation-develop-20260925-t0-t2-followup/focused-suite.log) | Preserve the location-only recovery boundary and continue the remaining route matrix with exact source, coverage, and render evidence. |
...
| `agent.map-render-ack-recovery` | `PARTIAL` | Candidate map preparation, browser acknowledgement, failed-render recovery, supersession, and last-known-good preservation. | Current exact-lane runs now complete the Colosseum recovery and USGS render acknowledgement; the manual-ready branch clears a prior basemap warning and returns to `Agent ready`. At `develop@66008da`, the visible MapLibre module also passes its happy path and all eight controlled fault cases; live FEMA/ESA sources still fail their separate checks. | `ISSUE-002`: public raster source failures remain; NOAA non-renderable evidence still needs a supported exact-lane handling path. | Public source-load behavior and last-known-good behavior with those public overlays prevent a complete claim. | 2026-09-26 | E2E | [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md), [browser evidence](../QA/tier3-validation-develop-20260926-map-rendering/browser-evidence.md), [exact-head controlled module](../QA/tier2-validation-develop-20260924-head66008da/controlled-fault/full-module.log), [hazard rerun](../QA/aegis-hazard-hydrology-e2e-20260920/report.md) | Investigate FEMA/ESA source loading and non-renderable alert handling; validate composition/retention with browser-visible public overlays while keeping the controlled acknowledgement module passing. |
...
| `maps.state-preservation` | `PARTIAL` | Overlay removal, visible-state narration, basemap preference, composition, and retaining an existing overlay during changes. | Current USGS browser evidence confirms live layer hide/show and retained basemap/map state; FEMA + USGS composition and FEMA retention remain blocked because FEMA never rendered. | `ISSUE-002`: retention of an unrendered FEMA overlay is not proven. | FEMA raster render is required before the composition and retention flows can complete. | 2026-09-26 | E2E | [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md), [browser evidence](../QA/tier3-validation-develop-20260926-map-rendering/browser-evidence.md), [state preservation](ui/state_preservation.md), [hazard rerun](../QA/aegis-hazard-hydrology-e2e-20260920/report.md) | Re-run composition and selective removal only after a verified FEMA source/layer acknowledgement. |
...
| `validation.application-foundations-tier1` | `VALIDATED` | Tier 1 route, state, conversation, realtime, run, API, jobs, Settings, credential, model, and context foundations. | T1-10 passed on `develop@8e32f82e3f094e5fb17c3978fad69b9f80b7af0b`; Tier 1 remains 12 `PASS`. The current campaign roll-up is 26 `PASS` / 1 `PARTIAL` / 1 `BLOCKED` / 40 `UNRUN` after the Tier 3 follow-up. The pushed head `9b34c5aa52dd6c26c9c451d0d3384fa660e5893d` passed all four jobs in [hosted CI run 36241506428](https://github.com/CTCycle/AEGIS-Geospatial-View/actions/runs/36241506428). | `T0-04` remains partial for its timing comparator; broader downstream live/browser/provider coverage remains open. | - | 2026-09-26 | integration/E2E | [validation strategy](validation/strategy.md), [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md), [Tier 3 browser evidence](../QA/tier3-validation-develop-20260926-map-rendering/browser-evidence.md), [Tier 1 checklist](validation/tier1_application_foundations.md) | Complete the remaining partial and unrun slices while keeping browser, provider, and exact-head CI boundaries distinct. |
...
| `validation.environment-startup-tier0` | `PARTIAL` | Tier 0 environment, current SQLite/Alembic schema, legacy Settings migration, Windows startup, and API composition slices. | The exact-head follow-up passes fresh/warm official-launcher readiness, simulated provider-outage startup, changed-owner/PID-reuse protection, injected backend/frontend readiness-failure cleanup, canonical-state protection, and final port cleanup. The current campaign is 26 `PASS`, 1 `PARTIAL`, 1 `BLOCKED`, and 40 `UNRUN`. | The safe historical before/after timing comparator remains unavailable; T0-04 stays partial for that timing limitation only. Downstream validation also remains partial or unrun. | — | 2026-09-26 | integration | [follow-up report](../QA/tier0-tier2-validation-develop-20260925-t0-t2-followup/report.md), [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md), [timing results](../QA/tier0-tier2-validation-develop-20260925-t0-t2-followup/timing-results.md), [safety harness](../QA/tier0-tier2-validation-develop-20260925-t0-t2-followup/launcher-safety-harness.json), [gate ledger](validation/gate_status.md) | Preserve the functional startup safety boundary; revisit timing only with a safe paired baseline. |
| `testing.complete-live-matrix` | `PARTIAL` | Complete live provider, browser, raster, composition, and coverage scenario matrix. | The controlled acknowledgement module passes 10/10; exact-lane browser evidence now also passes the current Colosseum recovery, USGS 48-feature clustered overlay, layer visibility mutation, and Dark/OpenTopoMap basemap switches. The current NOAA alert boundary is blocked after non-renderable provider evidence and repeated invalid tool calls. | Explicit gaps remain for public raster loading, route aliases and semantics, provider parity, credentialed/local sources, dataset ingestion, and other catalog families. General Overpass reliability remains partial; the safe T0-04 timing comparator remains unavailable. | External providers and configured sources vary by environment; T0-04 timing remains partial. | 2026-09-26 | E2E + automated | [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md), [browser evidence](../QA/tier3-validation-develop-20260926-map-rendering/browser-evidence.md), [follow-up report](../QA/tier0-tier2-validation-develop-20260925-t0-t2-followup/report.md), [controlled module](../QA/tier2-validation-develop-20260924-head66008da/controlled-fault/full-module.log) | Continue the remaining route, raster, provider, ingestion, recovery, and release rows with exact provider/model and browser-authoritative evidence. |
| `ci.hosted-exact-head` | `VALIDATED` | Hosted CI result for an exact pushed branch head. | Pushed head `9b34c5aa52dd6c26c9c451d0d3384fa660e5893d` passed all four jobs on [GitHub Actions run 36241506428](https://github.com/CTCycle/AEGIS-Geospatial-View/actions/runs/36241506428). | Live-browser status remains separately classified; hosted CI does not replace browser-authoritative provider evidence. | — | 2026-09-26 | hosted CI | [gate status](validation/gate_status.md), [Tier 3 report](../QA/tier3-validation-develop-20260926-map-rendering/report.md), [GitHub Actions run 36241506428](https://github.com/CTCycle/AEGIS-Geospatial-View/actions/runs/36241506428) | Rerun hosted CI on future source or test changes. |
