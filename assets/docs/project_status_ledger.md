...
| `ui.settings-provider-access` | `PARTIAL` | Settings sections for model selection, model-provider credentials, local runtime, geospatial access, and persisted application runtime configuration. | SQLite migration/API coverage, 177 targeted geospatial/chat regression tests, Angular production build, 30 focused Settings Karma tests, and an in-app browser pass across all seven sections pass; masked credentials, runtime save/restart notice, and the desktop model-card layout were visually verified. | Credential-save and explicit API failure states were not exercised with a safe fixture. | — | 2026-09-21 | integration | [frontend architecture](architecture/frontend_architecture.md), [access overview](geospatial/providers/access_overview.md), [user settings](user/settings_and_access.md), [runtime validation](../QA/settings-runtime-validation-20260921.md) | Exercise credential-save and failure states with safe fixture data before claiming full settings-flow coverage. |
...
| `ui.settings-provider-access` | `PARTIAL` | Credential-save and explicit API failure states beyond the completed section/navigation and masked-secret browser pass. | `MEDIUM` | Run the settings workflow with safe fixture credentials and controlled failure responses. |
