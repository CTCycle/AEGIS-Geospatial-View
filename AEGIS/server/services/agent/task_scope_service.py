...
class TaskScopeService:
    ...
    NEW_TASK_PATTERNS = (
        re.compile(r"\b(new task|different place|switch (?:location|city|area)|instead)\b", re.IGNORECASE),
    )
    ...
    CITY_OR_COUNTRY_RE = re.compile(r"\b(?:in|at|near)\s+[A-Z][a-z]+(?:[\s,]+[A-Z][a-z]+){0,2}\b")
    TIME_REFERENCE_RE = re.compile(r"\b(same time|same day|today|tomorrow|yesterday)\b", re.IGNORECASE)
    FILTER_REFERENCE_RE = re.compile(r"\b(same filters?|same layers?|same overlay|show traffic|show weather|show air quality)\b", re.IGNORECASE)
    ...
    def _latest_task_start_index(self, history: list[dict[str, Any]], *, current_user_index: int) -> int:
        start_index = 0
        for index, message in enumerate(history[:current_user_index]):
            ...
            if self._is_new_task_signal(content):
                start_index = index
                continue
            if self._has_new_location_cue(content) and not self._is_referential(content):
                start_index = index
        return start_index
    ...
    def decide_scope(
        ...
        *,
        ...
        starts_new_task = bool(explicit_new_task or (has_new_location and not referential))
        ...
        history_start_index = self._latest_task_start_index(history, current_user_index=current_user_index)
        return TaskScopeDecision(
            starts_new_task=False,
            carry_forward_location=has_persisted_location and (referential or not has_new_location),
            carry_forward_time=bool(referential or self.TIME_REFERENCE_RE.search(user_message)),
            carry_forward_filters=bool(referential or self.FILTER_REFERENCE_RE.search(user_message)),
