...
###############################################################################
def _merge_location(
    base: ExtractedLocation, patch: ExtractedLocation | None
) -> ExtractedLocation:
    if patch is None:
        return base
...
def _merge_time(
    ...
    return ExtractedTimeReferences(
        ...
        start_time=list(patch.start_time)
        if "start_time" in fields_set
        else list(base.start_time),
        end_time=list(patch.end_time)
        if "end_time" in fields_set
        else list(base.end_time),
    )


###############################################################################
def merge_extracted_intent(
    base: ExtractedIntent, patch: ExtractedIntentPatch
) -> ExtractedIntent:
    ...
    if (
        merged_coordinates.latitude is not None
        and merged_coordinates.longitude is not None
    ):
    ...
    return ExtractedIntent(
        ...
        if "location_type" in patch.model_fields_set
        else base.location_type,
        base_map_type=patch.base_map_type
        if "base_map_type" in patch.model_fields_set
        else base.base_map_type,
        time_references=_merge_time(base.time_references, patch.time_references),
        user_goal=patch.user_goal
        if "user_goal" in patch.model_fields_set
        else base.user_goal,
        filters=list(patch.filters)
        if "filters" in patch.model_fields_set
        else list(base.filters),
        area_of_interest=patch.area_of_interest
        if "area_of_interest" in patch.model_fields_set
        else base.area_of_interest,
        certainty=patch.certainty
        if "certainty" in patch.model_fields_set
        else base.certainty,
    )
