...
def _usage(
    *,
    ...
    return ContextUsageResponse(
        ...
        usage_percent=round(reported_input / 3072 * 100, 1),
        provider="test",
        model="runtime-model",
...
def test_phase_usage_reports_peak_request_and_preserves_all_phase_totals() -> None:
    ...
    assert result.context_usage.peak_request_tokens == 700
    assert result.context_usage.total_input_tokens == 1000
    assert result.context_usage.total_output_tokens == 23
    assert result.context_usage.usage_percent == round(700 / 3072 * 100, 1)
    assert set(result.context_usage.phases) == {"parser", "native_loop", "synthesis"}
    assert result.context_usage.phases["native_loop"]["reported_input_tokens"] == 700
    assert result.context_usage.phases["native_loop"]["reported_output_tokens"] == 11
