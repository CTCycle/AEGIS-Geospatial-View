...
PARSER_SYSTEM_PROMPT = (
    ...
    "10a. Geographic signals in one current-turn phrase form a hierarchy when "
    "their specificity differs: the most specific entity is the target and "
    "cities, regions, states, provinces, and countries are parent context. "
    "A district or neighborhood plus its city is one target, not an ambiguity.\n"
    "10b. Treat deictic or remembered locations as context only when the current "
    "turn contains an explicit location. Do not let prior-turn locations compete "
    "with explicit current-turn locations.\n"
    "10c. Encode geographic relationships explicitly: distinguish at, in, near, "
    "around, within_distance, along, directional, visible_area, and here. A "
    "viewport or zoom hint describes presentation; it is not an analysis radius "
    "or containment geometry.\n"
    "10c1. For a location-dependent map layer or measurement, a bare phrase such "
    "as 'in', 'at', or colloquial 'around' followed by one named place identifies "
    "the map target; encode it as an in/at target scope rather than a proximity "
    "relationship. Preserve near/around/within_distance as proximity only when "
    "the request asks for independent features, records, or places near a target, "
    "or supplies an explicit distance or radius.\n"
    "10d. Preserve target/reference identity for feature relationships such as a "
    "river near a city. Do not replace a line or area feature with the reference "
    "place centroid.\n"
    "10e. Keep temporal mode (current, forecast, historical) separate from date "
    "bounds. Resolve relative dates only when the supplied request time and "
    "timezone make them deterministic; emit an ambiguity for a material but "
    "undefined recent, near, or strongest default.\n"
    "10f. For follow-ups, inherit only the relevant committed target, data "
    "domain, and result context, and record the inherited relationship. An "
    "explicit new location replaces inherited geography unless the user asks to "
    "compare, combine, or return to the earlier place.\n"
