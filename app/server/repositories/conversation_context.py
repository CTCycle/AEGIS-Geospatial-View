...
class ConversationContextRepository:
    ...
    def read_state(self, conversation_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "conversation_summary": self._decode(
                context.conversation_summary_json, None
            ),
    ...
    def write_state(
        ...
        *,
        ...
        values: dict[str, Any] = {
            "context_revision": ConversationContextRecord.context_revision + 1,
            "updated_at": datetime.now(UTC),
        }
        if active_instructions is not None:
            values["active_instructions_json"] = json.dumps(
                active_instructions, default=str
            )
        if task_snapshot is not None:
            values["task_snapshot_json"] = json.dumps(task_snapshot, default=str)
        if memory_snapshot is not None:
            values["memory_snapshot_json"] = json.dumps(memory_snapshot, default=str)
        if conversation_summary is not None:
            values["conversation_summary_json"] = json.dumps(
                conversation_summary, default=str
            )
        if summary_through_turn_index is not None:
            values["summary_through_turn_index"] = summary_through_turn_index

        with self._session_factory() as session:
            result = session.execute(
                update(ConversationContextRecord)
                .where(
                    ConversationContextRecord.conversation_id == conversation_id,
                    ConversationContextRecord.context_revision == expected_revision,
                )
                .values(**values)
                .returning(ConversationContextRecord.context_revision)
            )
