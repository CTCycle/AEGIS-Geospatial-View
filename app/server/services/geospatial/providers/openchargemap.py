    ...
    async def fetch(self, request: ProviderRequest) -> ProviderResponse:
        ...
        if not api_key:
            raise ProviderAuthError(
                "Open Charge Map hosted API access requires a saved API key in encrypted Settings storage; configure it under Geospatial Access or use a local snapshot for anonymous operation."
            )
        params["key"] = api_key
