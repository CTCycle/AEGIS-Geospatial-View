...
class MobilityDatabaseProvider(GeospatialProvider):
        ...
        self.catalog_path = Path(
            ...
            or os.getenv(
                ...
                str(
                    ...
                    / "geospatial"
                    / "mobility_database_feeds_v2.csv"
                ),
            )
        ).expanduser()
    ...
    async def fetch(self, request: ProviderRequest) -> ProviderResponse:
        ...
        feeds.sort(
            key=lambda item: (
                str(item.get("name") or item.get("provider") or "").casefold(),
                str(item.get("id") or ""),
            )
        )
        feeds = feeds[:limit]
    ...
    async def _load_rows(self, *, refresh: bool) -> list[dict[str, str]]:
        ...
        try:
            return list(
                csv.DictReader(
                    io.StringIO(self.catalog_path.read_text(encoding="utf-8-sig"))
                )
            )
        except (OSError, UnicodeError, csv.Error) as exc:
            raise ProviderUnavailableError(
                "Mobility Database catalog is malformed."
            ) from exc
    ...
    def _normalize_row(self, row: dict[str, str]) -> dict[str, Any] | None:
        ...
        if not source_id and not provider:
            return None
        bbox = {
            "minimumLatitude": _float(
                _first(
                    row, "location.bounding_box.minimum_latitude", "minimum_latitude"
                )
            ),
            "maximumLatitude": _float(
                _first(
                    row, "location.bounding_box.maximum_latitude", "maximum_latitude"
                )
            ),
            "minimumLongitude": _float(
                _first(
                    row, "location.bounding_box.minimum_longitude", "minimum_longitude"
                )
            ),
            "maximumLongitude": _float(
                _first(
                    row, "location.bounding_box.maximum_longitude", "maximum_longitude"
                )
            ),
        }
        authentication_type = (
            _first(row, "urls.authentication_type", "authentication_type") or "0"
        )
        return {
            "id": source_id,
            ...
            "subdivisionName": _first(
                row, "location.subdivision_name", "subdivision_name"
            ),
            ...
            "staticFeedUrl": _first(
                row, "urls.latest", "latest", "latest.url", "urls.direct_download"
            ),
            ...
            "authentication": {
                ...
                "infoUrl": _first(
                    row, "urls.authentication_info_url", "authentication_info_url"
                ),
                "apiKeyParameterName": _first(
                    row, "urls.api_key_parameter_name", "api_key_parameter_name"
                ),
            },
            "realtimeUrl": _first(
                row, "urls.direct_download_url", "direct_download_url"
            ),
            "bbox": bbox,
        }
    ...
    def _matches(
        ...
        *,
        ...
        if bbox:
            ...
            if all(
                value is not None for value in (min_lon, max_lon, min_lat, max_lat)
            ) and (
                float(str(max_lon)) < west
                or float(str(min_lon)) > east
                or float(str(max_lat)) < south
                or float(str(min_lat)) > north
            ):
                return False
        return True
