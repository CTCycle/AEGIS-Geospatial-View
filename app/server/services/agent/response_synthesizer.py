...
class GroundedResponseSynthesizer:
    ...
    async def synthesize_async(
        ...
        *,
        ...
        skip_verified_map_only: bool = False,
    ) -> str:
        """Cancellable synthesis boundary for the async agent runtime."""

        if operation.kind in {"error", "rejection", "clarification"} or operation.status == "failed":
            return fallback_text
        if (
            skip_verified_map_only
            and operation.kind == "map_session"
            and map_session is not None
            and direct_result is None
            and clarification_plan is None
        ):
            return fallback_text
        if not self.enabled:
            return fallback_text
        ...
        try:
            provider = self.llm_factory.get_provider(settings.agent_model_provider)
            deadline = (
                execution_budget.stage_deadline("response_synthesis")
                if execution_budget is not None
                else monotonic() + self.SYNTHESIS_TIMEOUT_SECONDS
            )
            timeout = max(
                0.001,
                min(
                    (
                        ...
                        if execution_budget is not None
                        else self.SYNTHESIS_TIMEOUT_SECONDS
                    ),
            ...
            with observation:
                ...
                async_synthesis = getattr(provider, "astructured_output", None)
                async with asyncio.timeout(timeout):
                    if callable(async_synthesis):
                        payload = await cast(
                            Callable[..., Awaitable[dict[str, Any]]],
                            async_synthesis,
                        )(
                            request, GroundedSynthesisResult
                        )
                    else:
            ...
            self.last_failure_category = "provider_api"
            self.last_failure_detail = "Synthesis was cancelled with the active request."
            raise
        except TimeoutError:
            self.last_failure_category = "provider_api"
            self.last_failure_detail = "Synthesis exceeded the bounded application deadline."
            LOGGER.warning("Grounded response synthesis reached its application deadline")
            return fallback_text
        except LLMStructuredOutputError as exc:
            usage = getattr(exc, "context_usage", None)
            if is_json_object(usage):
...
async def synthesize_response_async(
    ...
    **kwargs: Any,
    ...
    """Use the cancellable synthesis boundary for the agent runtime.

    Production uses ``synthesize_async``.  A narrow thread fallback keeps
    injected legacy adapters usable while they migrate to that boundary; the
    fallback is never used by the built-in synthesizer.
    """

    async_method = getattr(synthesizer, "synthesize_async", None)
    if callable(async_method):
        return await cast(Callable[..., Awaitable[str]], async_method)(**kwargs)

    sync_method = getattr(synthesizer, "synthesize", None)
    if not callable(sync_method):
        raise TypeError("The response synthesizer has no synthesis method.")
    ...
    return await asyncio.to_thread(cast(Callable[..., str], sync_method), **filtered_kwargs)
